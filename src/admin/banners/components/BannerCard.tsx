...
import { useBannerDeleteMutation } from "../hooks/useBannerDeleteMutation";
...
export const BannerCard = ({ banner }: BannerCardProps) => {
  ...
  const fullImageUrl = `${VITE_API_URL_BASE}/${banner.image}`;

  const { deleteBannerMutation } = useBannerDeleteMutation();
  ...
  return (
    <Card className="overflow-hidden shadow-card hover:shadow-lg transition-all duration-300 animate-fade-in bg-card border-border group py-0 gap-2">
      ...
      <CardHeader className="pb-0 grid-cols-none grid-rows-none">
        <h3 className="text-lg font-semibold text-card-foreground">{banner.title}</h3>
      </CardHeader>
      ...
      overflow-hidden
    "
          dangerouslySetInnerHTML={{ __html: normalizeHtml(banner.slogan) }}
        />
      </CardContent>
      ...
      <CardFooter className="p-3 flex gap-2">
        ...
        <Button
          ...
          className={`text-destructive hover:text-destructive bg-destructive/10 hover:bg-destructive/20 flex-1 ${
            deleteBannerMutation.isPending ? "bg-gray-400 cursor-not-allowed text-white" : ""
          }`}
