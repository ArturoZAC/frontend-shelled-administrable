...
import { toast } from "sonner";
import { editBannerSchema, type EditBannerForm } from "../../schemas/edit-product.schema";
import { useEditBannersMutation } from "./hooks/useEditBannerMutation";
import { useOneProductQuery } from "./hooks/useOneBannerQuery";
...
export const EditProductsPage = () => {
  ...
  const oneServiceQuery = useOneProductQuery(+idProduct!);
  const editMutation = useEditBannersMutation(+idProduct!);
  ...
  if (oneServiceQuery.error)
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 bg-background">
        <p className="text-xl font-semibold text-destructive">Producto no encontrado</p>
  ...
  if (!oneServiceQuery.data)
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-4 min-h-screen">
        ...
        <p className="text-lg font-medium text-muted-foreground">Cargando Producto...</p>
      </div>
    );
