...
import { ProductCard } from "./components/BannerCard";
import { useProductQuery } from "./hooks/useProductQuery";

export const ProductsPage = () => {
  const navigate = useNavigate();

  const { getAllProductQuery } = useProductQuery();

  const handleNavigateAgregar = () => {
    navigate("add");
  };

  const banners = getAllProductQuery.data ?? [];

  if (getAllProductQuery.isLoading) {
    return (
  ...
  if (getAllProductQuery.isError) {
    return (
      <p className="text-center py-16 text-destructive">
        Error al cargar los Productos:{" "}
        {getAllProductQuery.error instanceof Error
          ? getAllProductQuery.error.message
          : "Algo salió mal"}
      </p>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 animate-fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Gestiona tus Productos disponibles</p>
            </div>
          </div>
        ...
        {banners.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center animate-fade-in">
            ...
            <p className="text-muted-foreground mb-6 max-w-sm">
              Comienza añadiendo tu primer producto para mostrarlo aquí.
            </p>
